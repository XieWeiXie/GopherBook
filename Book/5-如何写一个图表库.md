...
本章会介绍下常见的开源图表库以及相应的说明，实践环节会基于 chart.js 说明，如何结合 Golang，开发一款图表库：go-chart，结合网络爬虫获取到的数据，利用开发的 go-chart 进行数据的展示。
...
主要引入 React， Bable， BizChart 这边使用的都是对应的网络分发地址。
...
chart.js 是一个图表控件集合，使用html5的canvas进行绘制，支持所有现代浏览器，不依赖任何外部工具库，非常轻量，后续也会针对 chart.js 开发一个图表库：go-chart。
...
模版文件内的变量使用 `{{}}` 标示，当然还支持比如：判断、变量、管道等方法。
...
将动态数据加载入模版文件呢，给我们的启发是，构建图表环节需要将动态数据加载在 script 标签内，这个时候的操作完全可以是模版引擎达到目的。
...
使用 go 构建图表，核心的思想是：使用模版引擎加载动态数据。各类库的语法有差异，但构建图表的核心有三部分：1. 图表类型：比如饼图、折线图、柱状体、散点图、面积图等 2. 数据：不同的数据图表，数据的格式不同，比如散点图是(x,y) 型，折线图是(x)型。3. 配置项，这些配置项有些是全局的，有些是图表的特有的，常见的配置项有：坐标轴、标题、数据提示框、图例等。
...
定义 Interface 即方法的集合，所有的图表类型都应该实现这个 Interface。
