...
暂且先抛开 Go 本身的语法，我们从自己“创造编程”的角度来思考，需要考虑哪些方面？需要哪些知识呢？
...
上文我们组织的这些内容，其实称之为编程语言的基本语言特性，每们语言都具有这些内容，不管你学Python，Java，JavaScript 或者本书的主题 Go。明白这些内容之后，我们学习 Go 语言，甚至学习其他编程，都可以事半功倍。
...
命名是个很简单的事，但系统复杂之后，命名是个特别重要的事，所以这里建议，命名还是需要遵守一些规范。这样使得代码的可读性增强，对于后期的维护、修改，具有很大的帮助。
...
那哪种合适呢？建议使用显式的命名方式，这样阅读的可读性更好。阅读的过程就知道数据的类型。为什么？有时候赋值来自于函数，这时候，你不阅读函数的定义，不太明白变量的类型。但好的命名，使读者知道操作对象的类型，且知道操作对象的具体含义。
...
上文讲了变量的命名。关于变量还存在一个比较重要的内容，即变量的作用域，即变量的生效的范围。
- 局部作用域
- 全局作用域
...
变量是运行过程中会变动的量，所以，变量不适合做全局变量，因为运行过程中会改变，如果项目大了，多次使用到这个值，在运行过程中发生了变动，追踪起来不是很容易。

所以，避免使用变量的作用域作为全局变量。

相对于变量，存在一种不能变动的量，称之为常量。常量因为不会在运行中变动，所以适合用于全局范围。
...
表示一组不同数据类型的值的集合。基本定义和 C 语言内的结构体很像，但是用法和C差别很大。
```
struct
```
- 函数类型
